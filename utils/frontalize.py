...
def frontalize(img, proj_matrix, ref_U, eyemask=None):
    ACC_CONST = 800
    ...
    ind = np.ravel_multi_index((np.asarray(temp_proj2[1, :].round(), dtype='int64'), np.asarray(temp_proj2[0, :].round(), dtype='int64')),\
    ...
    synth_frontal_acc = synth_frontal_acc.reshape(tuple(img.shape[:2]), order='F')
    ...
    mapX = np.reshape(mapX,(-1,img.shape[1]), order = 'F')
    mapY = np.reshape(mapY,(-1,img.shape[0]), order = 'F')

    frontal_raw = cv2.remap(img, mapX, mapY, cv2.INTER_CUBIC)

    frontal_raw = frontal_raw.reshape((-1,img.shape[2]), order = 'F')
    frontal_raw[badind,:] = 0
    frontal_raw = frontal_raw.reshape(img.shape, order = 'F')
    return frontal_raw
    # # which side has more occlusions?
    # midcolumn = np.round(ref_U.shape[1]/2).astype(int)
