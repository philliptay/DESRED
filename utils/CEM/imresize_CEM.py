...
def imresize(im, scale_factor=None, output_shape=None, kernel=None,align_center=False, return_upscale_kernel=False,use_zero_padding=False,antialiasing=True, kernel_shift_flag=False):
    ...
    imresize.warnings = getattr(imresize,'warnings',set())
    ...
    def filter2d(input,special_padding_size=None):
        ...
        if antialiasing_kernel.size > 1000:
            cur_warning = 'Using GPU for image resizing (since kernel is of size %dx%d)' % (antialiasing_kernel.shape[0], antialiasing_kernel.shape[1])
            if cur_warning not in imresize.warnings:
                print(cur_warning)
                imresize.warnings.add(cur_warning)
            return (255*torch.nn.functional.conv2d(torch.from_numpy(input).type(torch.double).cuda().unsqueeze(0).unsqueeze(0)/255,
                torch.from_numpy(1 * np.rot90(antialiasing_kernel, 2)).unsqueeze(0).unsqueeze(0).type(torch.double).cuda(),
